...
check_dependencies() {
    ...
    # Check for Claude (claude command)
    if command -v claude &> /dev/null; then
        log_success "Claude found - will generate AI-powered changelog"
        CLAUDE_CODE_AVAILABLE=true
    else
        log_info "Claude not found - changelog generation disabled"
...
# Generate AI-powered changelog using Claude
generate_changelog() {
    local current_version="$1"
    local new_version="$2"
    
    if [ "$CLAUDE_CODE_AVAILABLE" != "true" ]; then
        log_info "Skipping changelog generation (Claude not available)"
        return 0
    fi
    
    log_info "Generating AI-powered changelog..."
    ...
    # Create changelog prompt for Claude
...
\`\`\`"
    ...
    if echo "$changelog_prompt" | claude --print > "$changelog_file"; then
        log_info "Claude executed successfully"
    else
        claude_exit_code=$?
        log_warning "Claude failed with exit code $claude_exit_code"
    fi
    ...
    if [ $claude_exit_code -eq 0 ] && [ -s "$changelog_file" ]; then
        ...
        log_warning "Failed to generate changelog, using basic git log"
...
🤖 Automated release preparation"; then
...
This changelog was automatically generated by analyzing git commits and file changes.
...
---
*Generated with AI Release Automation*"
